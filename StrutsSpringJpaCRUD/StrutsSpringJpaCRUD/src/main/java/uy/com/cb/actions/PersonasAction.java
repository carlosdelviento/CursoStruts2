...
import uy.com.cb.capaservicio.PersonaService;
...
public class PersonasAction extends ActionSupport {
	...
	@Action(value = "/guardarPersona", results = {
			@Result(name = "success", location = "listar", type = "redirect"),
			@Result(name = "input", location = "/WEB-INF/content/persona.jsp")
			})
	public String guardar() {
		...
		if (persona.getNombre().length() > 1) {
			addActionMessage(getText("nombre.valido"));
		}else {
			addFieldError("persona.nombre", getText("nombre.invalido"));
			return INPUT;
		}
		if (persona.getApellidoPaterno().length() > 1) {
			addActionMessage(getText("apePat.valido"));
		}else {
			addFieldError("persona.apellidoPaterno", getText("apePat.invalido"));
			return INPUT;
		}
		if (persona.getApellidoMaterno().length() > 1) {
			addActionMessage(getText("apeMat.valido"));
		}else {
			addFieldError("persona.apellidoMaterno", getText("apeMat.invalido"));
			return INPUT;
		}
		if (persona.getEmail().length() > 3) {
			addActionMessage(getText("email.valido"));
		}else {
			addFieldError("persona.email", getText("val.email.min.lenght"));
			return INPUT;
		}
		return SUCCESS;
	}
